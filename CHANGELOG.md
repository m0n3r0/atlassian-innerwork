...
- `tests/test_portability_stream.py` — 30 tests: the byte-identity referee (unicode/control chars, empty store, multi-batch >batch_size, batch_size=1, compact `indent=None`), `fetchall` ban (wrapped-connection proxy), counts parity with `export_domain`, audit composition (v2 byte-identity, redaction, no-sink fail-before-write), portability-event-after-success, interrupted-sink records no event, round-trip import/reexport, progress-callback cadence (API, incl. audit multi-batch) + `--progress` stderr (CLI, no row content, silent without the flag) + `--help` documents it, the tracemalloc memory differential (streamed < 25% of memory-resident peak, ≥ 8 MiB floor), unreadable import input, and CLI atomicity (`test_cli_export_out_atomic_on_error` — fail-before-write sentinel preserved, no temp litter; `test_cli_export_out_atomic_on_mid_write_oserror` — OSError mid-stream, sentinel preserved, temp removed, exit 2).
...
### Added — Time-windowed metrics

- `src/innerwork/analytics.py` — additive `windowed_domain_rollup(store, *, window_start=None, window_end=None, principal=None)` returning `DomainWindowMetrics` (`to_dict()` → the top-level `"window"` object), plus helper dataclasses `ProjectCycleTime` / `PageWritesRollup` / `ContributorsRollup`. Four activity-over-window rollups over half-open `[start, end)` UTC windows: `state_counts` (transitions into each state; all `WORKFLOW_STATES` keys present, 0 for unused), `cycle_time_per_project` (done-transition cycle times; count/avg/min/max, `null` when 0, every readable project listed), `page_writes` (total_versions / pages_touched / by_space from `page_versions`, every readable space key present), `contributors` (distinct / by_actor, sorted). Bounds parse with stdlib `datetime.fromisoformat` after `Z` → `+00:00` normalization (no new dependency); naive flag values rejected; unparseable stored timestamps raise `AnalyticsError` naming table+value (loud, never silently skipped); parameterized SQL only. Permission model reused verbatim (same `can_read` gating); `contributors.by_actor` is an opt-in, documented exception to the no-user-identifiers doctrine (only present when a window flag is passed). `domain_rollup` / `project_rollup` / `space_rollup` untouched — no flags → byte-identical output.
- `src/innerwork/cli.py` — `innerwork metrics` gains `--window-start` / `--window-end` (ISO-8601 with explicit UTC offset, both optional; either one alone gives an unbounded window on the other side). With flags, output is the point-in-time rollup plus the additive `"window"` object with normalized echoed bounds; `AnalyticsError` (inverted window, malformed/naive bound, unparseable stored timestamp) → `error: <msg>` on stderr + exit 2, empty stdout, no partial JSON.
